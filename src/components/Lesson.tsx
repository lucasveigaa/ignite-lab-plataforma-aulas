...
export function Lesson(props: LessonProps) {
  ...
  const avalaibleDateFormatted = format(
    props.availableAt,
  ...
  return (
    <Link to={`/event/lesson/${props.slug}`} className="group">
      <span className="text-gray-300">{avalaibleDateFormatted}</span>
    ...
    <div className={`rounded border border-gray-500 p-4 mt-2 group-hover:border-green-500 ${isActiveLesson ? 'bg-green-500' : ''}`}>
        ...
        <strong className={`text-gray-200 mt-5 block ${isActiveLesson ? 'text-white' : ''}`}>{props.title}</strong>
      </div>
    </Link>
  );
}
